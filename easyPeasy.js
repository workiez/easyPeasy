	...
	// home page
		...
		case '/en-cartelera':
            ...
            movies.forEach(movie => {
                ...
                res.write('- ' + movie.title)
                res.write('\n')
                res.write('-Descripcion: '+ movie.overview)
                res.write('\n')
            });
            ...
            case '/mas-votadas':
                ...
                res.write('\n\n')
                let mayorSiete = movies.filter((movies) => {
                    return movies.vote_average >= 7;
                });
                let rating = mayorSiete.map(function (movies) {
                    return movies.vote_average
                })
                let ratingTotal = rating.reduce(function (acum, num) {
                    return acum + num
                });
                let promedio = ratingTotal / mayorSiete.length
                ...
                res.write('Total de peliculas: ' + mayorSiete.length)
                res.write('\n\n')
                res.write('Rating promedio : ' + promedio)
                res.write('\n\n')
                movies.forEach(movies => {
                    res.write('Titulo :' + movies.title)
                    res.write('\n\n');
                    res.write('Reseña : ' + movies.overview)
                    res.write('\n\n');
                    res.write('Rating : ' + movies.vote_average)
                    res.write('\n\n');
                });
